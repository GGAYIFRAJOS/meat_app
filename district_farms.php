<?php 
  ...
  $sql = "SELECT * FROM farm JOIN users on farm.farmer_id = users.id JOIN village ON farm.village_id = village.id  JOIN parish ON village.parish_id = parish.id JOIN subcounty ON parish.subcounty_id = subcounty.id JOIN districts ON subcounty.district_id  = districts.id WHERE user_role = 'farmer' AND district_id = '$district_id'";
