<?php 
  ...
  $sql = "SELECT * FROM insurance JOIN users ON insurance.farmer_id = users.id JOIN farm ON farm.farmer_id = users.id JOIN village ON farm.village_id = village.id JOIN parish ON village.parish_id = parish.id JOIN subcounty ON parish.subcounty_id = subcounty.id JOIN districts ON subcounty.district_id  = districts.id  WHERE district_id = '$district_id' AND status = 'processed'";
